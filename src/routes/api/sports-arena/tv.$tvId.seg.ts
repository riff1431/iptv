...
export async function handleSegRequest(request: Request, tvId: string): Promise<Response> {
  ...
  const { verifySegmentToken, rewritePlaylist } = await import(
    "@/lib/iptv-proxy.server"
  );
  ...
  if (!upstreamUrl) return new Response("Forbidden", { status: 403 });
        ...
        const RETRY = {
          playlist: { maxAttempts: breakerOpen ? 1 : playlistAttempts, baseDelayMs: playlistBase, maxDelayMs: playlistCap },
          segment: { maxAttempts: breakerOpen ? 1 : segmentAttempts, baseDelayMs: segmentBase, maxDelayMs: segmentCap },
        } as const;
        ...
        if (isPlaylist) {
            ...
            try {
              ...
              if (!isRetryableStatus(upstream.status)) {
                ...
                logFailure({
                  ...
                  attempts: attempt,
                  message: `${lastFailure.statusText ?? ""}${breakerOpen ? " [breaker:open]" : ""}${tripped ? " [breaker:tripped]" : ""}`.trim() || undefined,
                });
          ...
          logFailure({
            ...
            message: `${lastFailure?.message ?? ""}${breakerOpen ? " [breaker:open]" : ""}${tripped ? " [breaker:tripped]" : ""}`.trim() || undefined,
            durationMs: Date.now() - overallStart,
