...
function LoungePage() {
  ...
  return (
    <AppShell>
      <div className="min-h-screen bg-arena text-white">
        <div className="mx-auto max-w-[1600px] px-3 pt-3 sm:px-6 sm:pt-4">
          <ArenaHeader
            liveGames={lounge.tvs.length}
            viewers={viewers}
            title={lounge.name}
            leading={
              <Link
          ...
          <div className="relative isolate overflow-hidden rounded-3xl border border-arena-border">
            ...
            >
              ...
              <div className={`min-w-0 ${chatVisible ? "lg:flex lg:flex-col" : "hidden"}`}>
                <ArenaChatPanel loungeId={lounge.id} online={viewers} visible={chatVisible} />
              </div>
            </div>
