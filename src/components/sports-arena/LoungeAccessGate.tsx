...
export type LoungeAccessGateProps = {
  loungeId: string;
  children: (access: LoungeAccess) => React.ReactNode;
};

/**
 * Gates the lounge experience behind wallet-based entry:
 *   - Not entered → "Start free 2-min preview" (or free entry if fee=0)
 *   - Preview → renders children with a countdown pill; on expiry shows pay CTA
 *   - Paid → renders children with a paid countdown pill
 */
export function LoungeAccessGate({ loungeId, children }: LoungeAccessGateProps) {
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      setError(null);
      const a = await fetchAccess({ data: { loungeId } });
      setAccess(a);
    } catch (e) {
      setError(e instanceof Error ? e.message : "Failed to load access");
    }
  }, [fetchAccess, loungeId, authed]);
  ...
  // When preview expires locally, refetch so the pay prompt shows.
  useEffect(() => {
  ...
  if (error) {
    return (
      <GatePanel>
        ...
        <Button onClick={() => void refresh()} className="mt-3">Retry</Button>
      </GatePanel>
    );
  }
  ...
  // No active session — show entry CTA.
  if (!access.sessionId) {
    return (
      <GatePanel>
        <div className="flex items-center justify-center gap-2 text-primary">
        ...
        <p className="mt-2 text-sm text-muted-foreground">
          {access.entryFeeCents === 0
            ? "This lounge is free. Step inside and grab a seat."
        ...
        <div className="mt-4 flex flex-wrap justify-center gap-2">
          <Button
            ...
            onClick={async () => {
              setBusy("enter");
              try {
                const a = await enter({ data: { loungeId } });
                setAccess(a);
              } catch (e) {
  ...
  // Session is active — render the lounge, plus a countdown chip / pay prompt.
  const previewExpired = access.status === "preview" && (countdown?.seconds ?? 0) <= 0;
          ...
          <span className="ml-auto flex items-center gap-2">
            ...
            <Button
              ...
              onClick={async () => {
                setBusy("pay");
                try {
                  const a = await pay({ data: { loungeId } });
                  setAccess(a);
                } catch (e) {
